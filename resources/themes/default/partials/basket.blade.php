    ...
    <ul class="basket-list clearfix">
        @foreach($items as $item)
            <li class="col-xs-12" data-basket-row-id="{{$item->rowid}}">
                <div class="basket-item-image">
                    <a href="{!! $item->options['url'] !!}" target="_blank" title="{{$item->name}}" class="basket-item-thumb">
        ...
        @if(sizeof($items) && !$user)
            <li class="col-xs-12 text-center" data-question style="display: none;">
                ...
                <a href="{!! route('login') !!}" title="Перейти на страницу входа" class="btn btn-sm btn-purple col-xs-12" style="margin-bottom: 10px;">
                    Да, перейти на страницу входа
                </a>
                <a href="{!! route('reg') !!}" title="Перейти на страницу регистрации" class="btn btn-sm btn-purple col-xs-12">
...
<div class="modal-footer">
    ...
    <a class="btn btn-purple col-xs-12" @if($user) href="{!! route('get.order') !!}"  @else data-show-question @endif>
        Оформить заказ
    </a>
</div>
@endif
